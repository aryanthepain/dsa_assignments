...
// Get the row name for an entry in a specified column
string DataFrame::getRowName(size_t rowIndex, const string &colLabel) const
{
    // Check for out-of-bounds row index
    ...
    // Use iloc to retrieve the value at the specified row and column
    auto value = iloc(rowIndex, colIndex);
    if (holds_alternative<double>(value))
    {
        return to_string(get<double>(value)); // Return the row name as a string
    }
    else
    {
        return get<string>(value); // Return the row name as a string
    }
}

// Get the column name for an entry in a specified row
string DataFrame::getColumnName(size_t colIndex, const string &rowLabel) const
{
    // Find the index of the row label
    auto it = find(indexLabels.begin(), indexLabels.end(), rowLabel);
    if (it == indexLabels.end())
    ...
    // Check for out-of-bounds column index
    ...
    if (holds_alternative<double>(value))
    {
        return to_string(get<double>(value)); // Return the column name as a string
    }
    else
    {
        return get<string>(value); // Return the column name as a string
    }
}